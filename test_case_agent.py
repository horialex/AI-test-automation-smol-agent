...
test_case_agent.run(
   """
   can you please write BDD style UI test case using task "write_file" in for below requirement.
...
test_case_agent.run(
   ...
   rules: create single feature, include edge cases, use boundry value and equivalent partitioning test case creation technique. also, don't hallucinate or create madeup testcases.
          only assert the response code and not the response body, test only for 200 expected response code
